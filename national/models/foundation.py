...
class EncoderSpec:
    ...
    sge_gate_input: bool = False
    """Support-Gated Encodings (P-SGE, docs/research/
    2026-07-12_sge_preregistration.md). When True, the LAST input column
    is a per-row support gate g in (0, 1) — a FIXED geometric function of
    the training coordinates precomputed by
    ``national.data.sge_gate.fit_sge_gate`` (never trained). The column is
    STRIPPED from the covariates (it is a multiplier, not a feature) and
    multiplies both random-Fourier coordinate blocks:
    ``phi_SGE(x) = g(x) * phi_Fourier(x_std)``. g -> 0 reproduces the
    zero_fourier (fail-safe constant) representation exactly; g -> 1 the
    ungated one. The inducing points' trailing gate coordinate is frozen
    by a gradient hook in ``FoundationModel.__init__`` so no gate anywhere
    is a trained parameter."""

    sge_gate_bands: int = 0
    """MS-SGE per-band gates (P-MS, docs/research/
    2026-07-13_msge_preregistration.md). When > 0, the LAST
    ``sge_gate_bands`` input columns are per-band gates g_k in [0, 1] —
    one per Fourier band, so this must EQUAL ``fourier_bands`` — computed
    by ``national.data.sge_gate.fit_msge_gate`` (fixed geometric functions,
    never trained). Like the scalar gate, the columns are STRIPPED from
    the covariates; gate k multiplies exactly its own band's
    (sin, cos) x (lat, lon) feature quadruple. All-ones gates reproduce
    the ungated representation, all-zeros the zero_fourier one. Mutually
    exclusive with ``sge_gate_input``. The inducing points' trailing
    gate-band coordinates are frozen by the same gradient-hook mechanism
    as the scalar gate."""
...
class ResMLPEncoder(nn.Module):
    ...
    def __init__(self, spec: EncoderSpec) -> None:
        super().__init__()
        self.spec = spec
        if spec.sge_gate_bands < 0:
            raise ValueError(
                f"EncoderSpec.sge_gate_bands must be >= 0; got {spec.sge_gate_bands}"
            )
        if spec.sge_gate_input and spec.sge_gate_bands > 0:
            raise ValueError(
                "EncoderSpec.sge_gate_input (scalar SGE gate) and "
                "sge_gate_bands (MS-SGE per-band gates) are mutually "
                "exclusive — one gate mechanism per model."
            )
        if spec.sge_gate_bands > 0 and spec.sge_gate_bands != spec.fourier_bands:
            raise ValueError(
                "MS-SGE per-band gates are one-per-Fourier-band: "
                f"sge_gate_bands ({spec.sge_gate_bands}) must equal "
                f"fourier_bands ({spec.fourier_bands})."
            )
        n_gate_cols = 1 if spec.sge_gate_input else spec.sge_gate_bands
        min_input = 3 + n_gate_cols
        if spec.n_input < min_input:
            raise ValueError(
                f"EncoderSpec.n_input must be >={min_input} (lat, lon, depth"
                f"{f' + {n_gate_cols} trailing SGE gate column(s)' if n_gate_cols else ''}"
                f" + covariates); got {spec.n_input}"
            )
        ...
        # The trailing SGE gate column(s) (when present) are multipliers on
        # the Fourier block, NOT input features -- exclude from the width.
        n_covariates = spec.n_input - 3 - n_gate_cols
        ...
        # Eval-only SGE gate (the test_only arm): a per-row gate tensor set
        # via `set_eval_gate` right before prediction and applied to the
        # TRAILING len(gate) rows of the next forward passes (the
        # variational strategy prepends the inducing points, so the query
        # rows are always the trailing block -- same convention as
        # `_DKLApproximateGP.last_batch_encoded`). Never allowed in train
        # mode: the whole point of the arm is a train path bit-identical to
        # base. Plain attribute, not a buffer, so it never serializes.
        self._eval_gate: torch.Tensor | None = None

    def set_depth_scale(self, depth_scale_m: float) -> None:
        self._depth_scale_m = float(depth_scale_m)

    def set_eval_gate(self, gate: torch.Tensor | None) -> None:
        """Arm ``test_only``: install (or clear, with ``None``) a per-row
        support gate applied only at eval/predict time to the trailing
        ``len(gate)`` rows of each forward pass."""
        if gate is None:
            self._eval_gate = None
            return
        self._eval_gate = torch.as_tensor(gate).reshape(-1)
    ...
    def _featurize(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # SGE (P-SGE): the trailing input column is the precomputed support
        # gate -- multiply the Fourier block, strip it from the covariates.
        n_trailing = 0
        if self.spec.sge_gate_input:
            n_trailing = 1
            gate_col = x[:, -1].unsqueeze(-1)
            lat_feat = lat_feat * gate_col
            lon_feat = lon_feat * gate_col
        elif self.spec.sge_gate_bands > 0:
            # MS-SGE (P-MS): the trailing sge_gate_bands columns are per-band
            # gates. _FourierFeatures lays each axis out as
            # [sin_0..sin_{K-1}, cos_0..cos_{K-1}], so tiling the gate block
            # twice aligns gate k with exactly its own (sin_k, cos_k) pair on
            # both axes -- band k's full (sin, cos) x (lat, lon) quadruple.
            n_trailing = self.spec.sge_gate_bands
            band_gates = x[:, -n_trailing:]
            mult = torch.cat([band_gates, band_gates], dim=-1)
            lat_feat = lat_feat * mult
            lon_feat = lon_feat * mult

        # test_only arm: eval-time-only gate on the trailing query rows.
        if self._eval_gate is not None:
            if self.training:
                raise RuntimeError(
                    "set_eval_gate() gate present during a train-mode forward "
                    "-- the test_only arm must train exactly like base; clear "
                    "the gate (set_eval_gate(None)) before training."
                )
            g = self._eval_gate.to(device=x.device, dtype=x.dtype)
            if g.numel() > x.shape[0]:
                raise ValueError(
                    f"eval gate has {g.numel()} rows but the forward batch "
                    f"has only {x.shape[0]}"
                )
            mult = torch.ones(x.shape[0], 1, device=x.device, dtype=x.dtype)
            mult[x.shape[0] - g.numel():, 0] = g
            lat_feat = lat_feat * mult
            lon_feat = lon_feat * mult

        if x.shape[-1] > 3 + n_trailing:
            covariates = x[:, 3 : x.shape[-1] - n_trailing]
...
class _DKLApproximateGP(gpytorch.models.ApproximateGP):
    ...
    def __init__(
        ...
        # Cache of the most recent encoder forward, WITH its autograd graph
        # attached. `_VariationalStrategy.__call__` invokes `self.forward`
        # exactly once per `gp(x)` call (on `cat([inducing_points, x])`), so
        # this is set exactly once per forward pass. The trainer's
        # HSIC/MMD independence penalties reuse this tensor (see
        # `FoundationTrainer.fit`) instead of calling `encoder(x)` a second
        # time, which would otherwise double the BatchNorm running-stats
        # updates per optimizer step whenever those penalties are active.
        self._last_encoder_out: torch.Tensor | None = None

    def forward(self, x: torch.Tensor) -> gpytorch.distributions.MultivariateNormal:
        z = self.encoder(x)
        self._last_encoder_out = z
    ...
    def last_batch_encoded(self, batch_size: int) -> torch.Tensor:
        """Return the encoder output for the trailing ``batch_size`` rows of
        the most recent :meth:`forward` call.

        During training, GPyTorch's variational strategy calls
        ``forward(cat([inducing_points, x]))``, so ``x``'s encoded rows are
        the *last* ``batch_size`` rows of the cached tensor (inducing points
        come first in the concatenation -- see
        ``gpytorch.variational.VariationalStrategy.forward``). This lets
        callers recover exactly the encoder output that fed the GP loss for
        the current mini-batch, without a second encoder forward pass.
        """
        if self._last_encoder_out is None:
            raise RuntimeError(
                "last_batch_encoded() called before any forward() pass; "
                "call gp(x) (or model(x)) first."
            )
        return self._last_encoder_out[-batch_size:]
...
def _make_zero_trailing_columns_grad(n_cols: int):
    """Gradient-hook factory freezing the trailing ``n_cols`` (SGE gate)
    coordinates of the inducing points -- see ``FoundationModel.__init__``.
    ``n_cols`` is 1 for the scalar P-SGE gate and ``sge_gate_bands`` for the
    MS-SGE per-band gates."""
...
class FoundationModel(nn.Module):
    ...
    def __init__(self, spec: FoundationSpec, inducing_points: torch.Tensor) -> None:
        ...
        # SGE (P-SGE) / MS-SGE (P-MS): the trailing gate coordinate(s) of the
        # (otherwise learnable) inducing points must NOT train -- the
        # pre-registration's variance-bomb lesson is that the gate is a fixed
        # geometric function with "no tuned or trained parameters anywhere".
        # A gradient hook zeroes the gate column(s)' gradient every backward
        # pass. (Hooks are runtime-only and never serialize; reloaded
        # artifacts are used for prediction, where no gradient flows, so
        # nothing is lost on load.)
        n_gate_cols = 1 if spec.encoder.sge_gate_input else spec.encoder.sge_gate_bands
        if n_gate_cols > 0 and spec.svgp.learn_inducing:
            ip = self.gp.variational_strategy.inducing_points
            if isinstance(ip, torch.nn.Parameter):
                ip.register_hook(_make_zero_trailing_columns_grad(n_gate_cols))
    ...
    def predict_for_tta(
        ...
        """Differentiable, device-resident, standardised prediction.

        Mirror of :meth:`predict` for test-time-adaptation (Pillar 4)
        consumers. The two functional deltas are:

        * **no** ``@torch.no_grad()`` decorator -- gradients flow through
          the encoder + BatchNorm affines, which is what TENT and the
          self-training pseudo-label loop need.
        * outputs stay on the model's device and in standardised target
          space; the caller can choose whether to denormalise / move to
          CPU after their adaptation pass.

        The caller is responsible for setting ``model.train()`` if they
        want BatchNorm in train mode (the BN-stats adapter does); the
        default behaviour after construction is ``model.eval()``.
        """
        param = next(self.parameters())
        ...
        return FoundationPrediction(mean=mean, std=mean_std, encoded=encoded)
