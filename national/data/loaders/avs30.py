...
class AVS30Loader:
    ...
    def _lookup(self, lat: float, lon: float) -> float:
        ...
        try:
            ...
            # J-SHIS GeoJSON response shape is
            #   response.features[0].properties.AVS  (a stringified float in m/s,
            #   e.g. "302.9", or None when the mesh has no V_s30 estimate).
            # Earlier versions used `response.avs30` which never existed on the
            # Pydantic model -- the loader silently fell through to the fill
            # value on every API hit, producing an all-zeros warm cache.
            features = getattr(response, "features", None) or []
            avs_str: str | None = None
            if features:
                props = getattr(features[0], "properties", None)
                if props is not None:
                    avs_str = getattr(props, "AVS", None)
            val = float(avs_str) if avs_str else self._fill_value
        except (
            ValueError,
            TypeError,
            AttributeError,
            IndexError,
            Exception,  # noqa: BLE001 -- offline / API down / OOD
        ):
