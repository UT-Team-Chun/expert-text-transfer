...
class BoringDataset(Dataset[dict]):
    ...
    def __init__(
        ...
        *,
        ...
        extra_one_hot_columns = extra_one_hot_columns or {}
        missing_extra = [c for c in extra_one_hot_columns if c not in df.columns]
        if missing_required or missing_features or missing_extra:
            raise KeyError(
                f"Parquet at {path} is missing columns: required={missing_required!r}, "
                f"features={missing_features!r}, extra_one_hot={missing_extra!r}"
            )
        ...
        # Extra categorical one-hots (e.g. AIST era, lithology macro). The
        # dict's insertion order determines feature-vector ordering; downstream
        # code that reads `feature_names` can recover the layout from there.
        # Each column's codes are clipped to [0, dim-1]; the contract is that
        # the highest-index category is the UNKNOWN sentinel (see
        # `national.data.derived.aist_granular.AistEra.UNKNOWN` etc.) so NaN
        # rows -- which read as `dim` from `fillna(dim)` -- safely map there.
        extra_hot_names: list[str] = []
        extra_code_arrays: dict[str, np.ndarray] = {}
        for col, dim in extra_one_hot_columns.items():
            if dim < 1:
                raise ValueError(
                    f"extra_one_hot_columns[{col!r}] dim must be positive, got {dim}"
                )
            raw = df[col].to_numpy()
            # Cast through pandas to coerce NaN -> highest sentinel index.
            codes = (
                pd.Series(raw)
                .fillna(dim - 1)
                .astype(np.int64)
            ...
            clipped_c = np.clip(codes, 0, dim - 1)
            one_hot_c = np.eye(dim, dtype=np.float32)[clipped_c]
            pieces.append(one_hot_c)
            extra_hot_names.extend([f"{col}_oh_{i}" for i in range(dim)])
            extra_code_arrays[col] = clipped_c.astype(np.int16)
        ...
        self._regime = regime
        # Per-column code arrays for any extra one-hot categoricals. Mirrors
        # ``self._regime`` and is consumed by downstream conformal / sampler
        # code that wants to group by era / lithology / etc.
        self._extra_categorical: dict[str, np.ndarray] = extra_code_arrays
        ...
        self._standardize_target = bool(standardize_target)
    ...
    # ---- target-scaler refit (leak fix, codex review R4) ---------------------
    def _transformed_target(self) -> np.ndarray:
        """Recompute the post-transform, PRE-standardization target for all
        rows — bit-identical to the array built at construction time (same
        inputs, same ops, same float32 dtype), so a refit reproduces the
        construction-time scaler exactly when given every row."""
        y = self._y_raw
        if self._baseline_pred_per_row is not None:
            # Hybrid residual target: y - baseline, exactly as in __init__.
            y = (y - self._baseline_pred_per_row).astype(np.float32)
        return apply_target_transform(y, self._target_transform).astype(np.float32)

    def refit_target_stats(self, row_indices) -> tuple[float, float]:
        """Refit the target mean/sd on EXACTLY ``row_indices`` and
        re-standardize ``self._y`` (all rows) under the new scaler.

        This is the fix for the target-standardization leak found in the
        codex R4 review (docs/research/2026-07-13_codex_review_r4.md): the
        construction-time scaler is computed from ALL rows the dataset was
        given — in the training path the FULL parquet, i.e. INCLUDING the
        held-out region's labels. Callers doing spatial hold-out evaluation
        should pass the final training rows (post-exclusion, post-subsample,
        post-carve) so no held-out label statistics touch the scaler.

        The default (never calling this) is bit-exactly the historical
        behaviour — every result predating 2026-07-13 depends on it.

        Returns the fitted ``(mean, std)``.
        """
        if not self._standardize_target:
            raise RuntimeError(
                "refit_target_stats requires standardize_target=True — with "
                "standardization off there is no scaler to refit."
            )
        rows = np.asarray(row_indices, dtype=np.int64)
        if rows.ndim != 1 or len(rows) == 0:
            raise ValueError(
                f"row_indices must be a non-empty 1-D index array; got "
                f"shape {rows.shape}"
            )
        y_transformed = self._transformed_target()
        subset = y_transformed[rows]  # IndexError on out-of-range = fail loud
        # Identical estimator + epsilon as construction (float32 mean/std).
        self._target_mean = float(subset.mean())
        self._target_std = float(subset.std()) + 1e-6
        self._y = (
            (y_transformed - self._target_mean) / self._target_std
        ).astype(np.float32)
    ...
    @property
    def extra_categorical_codes(self) -> dict[str, np.ndarray]:
        """Per-row clipped codes for each extra one-hot categorical column.

        Mirrors :attr:`_regime`: an int16 array of length ``len(self)`` for
        each column passed via ``extra_one_hot_columns``. Useful for
        downstream consumers (e.g. Mondrian conformal recalibration grouped
        by AIST era / lithology macro instead of regime). Returns an empty
        dict if no extras were configured. The returned dict is a fresh
        copy so callers cannot mutate internal state.
        """
        return dict(self._extra_categorical)

    @property
    def regimes(self) -> np.ndarray:
        """Per-row integer regime codes (int16). Consumed by RegimeBalancedSampler."""
        return self._regime
