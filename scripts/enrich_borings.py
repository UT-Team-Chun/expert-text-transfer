...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Carry the borehole identity (boring_file = XML basename, "
            "dtd_version) from the input CSV into the parquet. Required for "
            "ID-exact text joins and borehole-grouped resampling; ~15%% of "
            "boreholes share byte-identical coordinates, so (lat, lon) is "
            "not a valid borehole key."
        ),
    )
